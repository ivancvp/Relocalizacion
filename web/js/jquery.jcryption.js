...
function cifrar(data) {
    return $.jCryption.encrypt(data, password);
}
$("html").hide();
//var $loader = $('<img src="loading.gif" alt="cargando..." title="cargando..." style="margin-right:15px;" />');
...
$(document).ready(function () {
    $.ajaxSetup({
        beforeSend: function (jqXHR, settings) {
            final = "", final_url = "";
            if (settings.encrypt) {
                final_url += settings.url + "?decryptData=true";
                if (settings.data) {
                    list_p = settings.data.split("&");
                    if (list_p) {
                        $(list_p).each(function (i, e) {
                            temp = e.split("=");
                            final += "&" + temp[0] + "=" + cifrar(temp[1]);
                        });
                        if (final.charAt(0) === '&') {
                            final = final.slice(1);
                        }
                        settings.data = final;
                        settings.url = final_url;
                    }
                }
            }
        }
    ...
    $.jCryption.authenticate(password, "encrypt?generateKeyPair=true", "encrypt?handshake=true",
            function (AESKey) {
                $("html").show();
//                $("#status").html('<span style="font-size: 16px;">Listones!</span>');
            },
            function () {
